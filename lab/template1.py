...
for i in range(total_exp):
    LOs.append(input(f'Enter the LO for Exp{i+1}: '))

print(LOs)  # Print to check the entries
...
for i in range(total_exp):
    ...
    sheet1.cell(row=5, column=i+3, value=LOs[i])  # Add "Exp 1", "Exp 2", etc., in row 5 across columns


# Adding columns for averages and assignments after the experiments
sheet1.cell(row=4, column=total_exp+3, value="Average(15)")
sheet1.cell(row=4, column=total_exp+4, value="Assignment 1")
sheet1.cell(row=4, column=total_exp+5, value="Assignment 2")
sheet1.cell(row=4, column=total_exp+6, value="Average Assignment(5)")
